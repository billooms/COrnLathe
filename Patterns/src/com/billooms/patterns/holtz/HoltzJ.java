...
/**
 * Holtzapffel J -- sine wave on top of a flower.
 *
 ...
 */
@ServiceProvider(service = Pattern.class, position = 200)
public class HoltzJ extends BasicPattern {
  ...
  public double getValue(double n, int r, int n2, double a2) {
    ...
    double deadZone = 0.3 / rr;	// TODO: The 0.3 is just a fudge to prevent a dimple in the middle
    if ((nn < deadZone) || (nn > (1.0 - deadZone))) {
      return this.getValue(deadZone, r, n2, a2);
    }
    double s = ampl2 * sine.getValue(nn * nn2);
    double outline = (1.0 - ampl2) * flower.getValue(nn, rr);
    return outline + s;
  }
}
