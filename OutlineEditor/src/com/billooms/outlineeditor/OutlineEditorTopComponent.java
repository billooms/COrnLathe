...
//	    + " " + evt.getPropertyName() + " " + evt.getOldValue() + " " + evt.getNewValue());
  ...
  private void writeOutlineData(double delta) {
    if (delta <= 0.0) {
      return;
    }
    ...
    if (fileObj != null) {
      String suffix = "";
      textFile = new File((fileObj.getPath()).replaceAll("." + fileObj.getExt(), suffix + "." + EXTENSION));
    } else {
    ...
    StatusDisplayer.getDefault().setStatusText("Saving Outline File As: " + textFile.getName());
    ...
    Curve cutSurface = new Curve(outline.getCutSurfaceCurve());
    cutSurface.reSample(delta);
    try {
      for (Point2D.Double pt : cutSurface.getPoints()) {
        out.println(F4.format(pt.getX()) + "\t" + F4.format(pt.getY()));
      }
    } catch (Exception e) {
  ...
  private void initComponents() {
    ...
    org.openide.awt.Mnemonics.setLocalizedText(jWriteButton, org.openide.util.NbBundle.getMessage(OutlineEditorTopComponent.class, "OutlineEditorTopComponent.jWriteButton.text")); // NOI18N
  ...
  private void writeData(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_writeData
    if (outline.getNumPts() >= 2) {
      ...
      double delta = ((Number) panel.deltaField.getValue()).doubleValue();

      writeOutlineData(delta);
    }
