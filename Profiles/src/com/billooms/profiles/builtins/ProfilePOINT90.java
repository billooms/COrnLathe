...
/**
 * A Profile that had a 90 degree point at the tip.
 *
 ...
 */
@ServiceProvider(service = Profile.class, position = 20)
public class ProfilePOINT90 extends BasicProfile {

  private final static double TAN45 = Math.tan(Math.toRadians(45.0));

  /**
   * Create a new profile with a 90 degree point.
   */
  public ProfilePOINT90() {
    super("Point90");
  }
  ...
  public double profileAt(double d, double rodRadius) {
    ...
    return Math.abs(d) / TAN45;		// symmetrical around center
  }
  ...
  public double widthAtDepth(double d, double rodDiameter) {
    if (d <= 0.0) {
      ...
      return Math.min(rodDiameter, 2.0 * d * TAN45);
    }
  }
}
