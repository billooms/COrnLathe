 ...
 */
@ServiceProvider(service = Profile.class, position = 40)
public class ProfilePOINT150 extends BasicProfile {

  private final static double TAN75 = Math.tan(Math.toRadians(75.0));

  /**
   * Create a new profile with a 150 degree point.
   */
  public ProfilePOINT150() {
    super("Point150");
  }
  ...
  public double profileAt(double d, double rodRadius) {
    ...
    return Math.abs(d) / TAN75;		// symmetrical around center
  }
  ...
  public double widthAtDepth(double d, double rodDiameter) {
    if (d <= 0.0) {
      ...
      return Math.min(rodDiameter, 2.0 * d * TAN75);
    }
  }
}
