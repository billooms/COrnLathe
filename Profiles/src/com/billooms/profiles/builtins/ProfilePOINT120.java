...
/**
 * A Profile that had a 120 degree point at the tip.
 *
 ...
 */
@ServiceProvider(service = Profile.class, position = 30)
public class ProfilePOINT120 extends BasicProfile {

  private final static double TAN60 = Math.tan(Math.toRadians(60.0));

  /**
   * Create a new profile with a 120 degree point.
   */
  public ProfilePOINT120() {
    super("Point120");
  }
  ...
  public double profileAt(double d, double rodRadius) {
    ...
    return Math.abs(d) / TAN60;		// symmetrical around center
  }
  ...
  public double widthAtDepth(double d, double rodDiameter) {
    if (d <= 0.0) {
      ...
      return Math.min(rodDiameter, 2.0 * d * TAN60);
    }
  }
}
