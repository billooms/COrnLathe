...
/**
 * A Profile that had a 60 degree point at the tip.
 *
 ...
 */
@ServiceProvider(service = Profile.class, position = 10)
public class ProfilePOINT60 extends BasicProfile {

  private final static double TAN30 = Math.tan(Math.toRadians(30.0));

  /**
   * Create a new profile with a 60 degree point.
   */
  public ProfilePOINT60() {
    super("Point60");
  }
  ...
  public double profileAt(double d, double rodRadius) {
    ...
    return Math.abs(d) / TAN30;		// symmetrical around center
  }
  ...
  public double widthAtDepth(double d, double rodDiameter) {
    if (d <= 0.0) {
      ...
      return Math.min(rodDiameter, 2.0 * d * TAN30);
    }
  }
}
