 ...
 */
@ServiceProvider(service = SpiralStyle.class, position = 60)
public class SpiralCOS4 extends AbstractSpiral {

  /**
   * Create a new spiral based on a quarter cosine wave.
   */
  public SpiralCOS4() {
    super("Cos4");
  }
  ...
  public void calculate(Double[] pts, double twist, double amp) {
    ...
    for (int i = 1; i < twistPts.length; i++) {
      ...
      twistPts[i] = new Point3D(pts[i].x, pts[i].y, twist * (1.0 - Math.cos((Math.PI / 2.0) * cum / tot)));
    }
  }
