...
/**
 * Spiral where a sine wave is superimposed on a UNIFORM_D. Note: this uses the
 ...
 */
@ServiceProvider(service = SpiralStyle.class, position = 40)
public class SpiralSINE extends AbstractSpiral {

  /**
   * Create a new spiral with a sine wave is superimposed on a UNIFORM_D. Note:
   * This uses the optional amplitude parameter.
   */
  public SpiralSINE() {
    super("Sine");
    needsAmplitude = true;
  ...
  public void calculate(Double[] pts, double twist, double amp) {
    ...
    for (int i = 1; i < twistPts.length; i++) {
      ...
      twistPts[i] = new Point3D(pts[i].x, pts[i].y, twist * cum / tot + amp * Math.sin(cum / tot * 2.0 * Math.PI));
    }
  }
