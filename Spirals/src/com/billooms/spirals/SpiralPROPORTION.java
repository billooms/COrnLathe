...
/**
 * Spiral where the twist is proportional to the radius. This gives more twist
 * at larger diameters.
 *
 ...
 */
@ServiceProvider(service = SpiralStyle.class, position = 30)
public class SpiralPROPORTION extends AbstractSpiral {

  /**
   * Create a new spiral with the twist proportional to the radius.
   */
  public SpiralPROPORTION() {
    super("Proportion");
  }
  ...
  public void calculate(Double[] pts, double twist, double amp) {
    ...
    for (int i = 1; i < pts.length; i++) {		// find the total length
      tot += pts[i].x * pts[i].distance(pts[i - 1]);
    }
    ...
    for (int i = 1; i < twistPts.length; i++) {
      cum += pts[i].x * pts[i].distance(pts[i - 1]);
